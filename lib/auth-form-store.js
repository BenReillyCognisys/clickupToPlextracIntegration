...
//
// postedDate: the calendar date (YYYY-MM-DD, in the auth-form timezone) the daily
// run last executed for. `ts` may be null when that run found nothing to post — the
// stamp still records that the run happened, so the reconcile can tell "no message
// yet today" from "already dealt with today" and post the first message if needed.
...
// Returns { channel, ts, entries, postedDate } of the last posted message, or null
// if none recorded. `entries` defaults to [] and `postedDate` to null for documents
// written before those fields existed.
async function getLastMessage() {
  const c = await col();
  const doc = await c.findOne({ _id: STATE_ID });
  return doc
    ? { channel: doc.channel, ts: doc.ts, entries: doc.entries || [], postedDate: doc.postedDate || null }
    : null;
}

// Records the message we just posted (and its per-task entries), replacing any
// previous record. `ts` may be null to record that the daily run happened on
// `postedDate` without posting anything (no qualifying tasks at the time).
async function setLastMessage(channel, ts, entries = [], postedDate = null) {
  const c = await col();
  await c.updateOne(
    { _id: STATE_ID },
    { $set: { channel, ts, entries, postedDate, updated_at: new Date() } },
    { upsert: true }
  );
}
