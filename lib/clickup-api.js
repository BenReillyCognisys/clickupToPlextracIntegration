...
/**
 * Marks a task checklist item resolved (or unresolved). This is the fallback for a
 * completion box modelled as a checklist item rather than a Checkbox custom field.
 * Note the path takes the CHECKLIST id, not the task id — both ids come off the
 * task's `checklists[].items[]`, which GET /task returns.
 */
async function setChecklistItemResolved(checklistId, itemId, resolved = true) {
  try {
    await axios.put(
      `${BASE}/checklist/${checklistId}/checklist_item/${itemId}`,
      { resolved },
      { headers: authHeaders(), timeout: CLICKUP_TIMEOUT_MS },
    );
  } catch (err) {
    throw clickupError(err, 'PUT', `/checklist/${checklistId}/checklist_item/${itemId}`);
  }
}
...
module.exports = {
  ...
  uploadCustomFieldAttachment, setChecklistItemResolved,
};
