...
function buildSystem({ clientName, doClientName, doDejargon, doSentences }) {
  ...
  if (doDejargon) {
    tasks.push(
      ...
      'facts, risk levels, severities and numbers. ' +
      'IMPORTANT: do NOT change standard penetration-testing engagement/scope terminology — leave ' +
      'these EXACTLY as written: black-box, grey-box (gray-box), white-box, internal, external, ' +
      'authenticated, unauthenticated, on-site, remote, red team, blue team, web application, ' +
      'infrastructure. These are precise terms of art the audience expects, not jargon to simplify. ' +
      'Record each edit as a change with type "dejargon".'
    );
  }
  if (doSentences) {
    tasks.push(
