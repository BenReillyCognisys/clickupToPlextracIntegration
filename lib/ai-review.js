...
async function correctClientName(text, correctName) {
  ...
  const system =
    ...
    'CRITICAL: never alter Plextrac template placeholders of the form %%...%% (e.g. ' +
    '%%CLIENT_FULLNAME%%, %%CLIENT_SHORTNAME%%, %%REPORT_START_DATE%%, %%Author_01%%) — leave ' +
    'them byte-for-byte unchanged; they are substituted by Plextrac at render time. ' +
...
async function deJargonExecutiveSummary(text) {
  ...
  const system =
    ...
    'findings. (4) Keep it concise and professional. (5) CRITICAL: never alter Plextrac ' +
    'template placeholders of the form %%...%% (e.g. %%CLIENT_SHORTNAME%%, %%REPORT_START_DATE%%, ' +
    '%%Author_01%%) — leave them byte-for-byte unchanged; Plextrac substitutes them at render ' +
    'time. If the summary is already suitable for a ' +
