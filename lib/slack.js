...
// Sends a direct message to a user. Passing a Slack user id (Uxxxxxxxx) as the
// `channel` makes chat.postMessage open/post to that user's IM. Requires the bot
// token to have the `chat:write` (and `im:write`) scopes.
async function postDirectMessage(userId, text) {
  return postMessage(userId, text);
}

module.exports = { postMessage, postReply, postDirectMessage };
