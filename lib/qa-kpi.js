...
async function buildKpiEntries(window = {}) {
  ...
  // Nothing recorded (e.g. no webhooks yet) — no cuids to resolve, so skip the
  // Plextrac user enumeration entirely rather than calling it for an empty board.
...
// "last 31 days", "Q2 2026 (Apr–Jun)") is shown in the header for context. Every
// consultant is listed (no cap) — sized comfortably for ~20 but the plain 1..N
// numbering and Slack's 40k-char text field let it scale well beyond that.
function renderKpis(entries, windowLabel) {
  ...
  const lines = entries.map((e, i) => {
    ...
    return `${i + 1}. ${slackEscape(e.name)} — *${e.count}* ${qa}${findingsSuffix(e)}`;
  });
...
// ── Per-user QA log (/qa-logs @user) ─────────────────────────────────────────

// Default and maximum number of recent QAs shown by /qa-logs.
const DEFAULT_LOG_LIMIT = 30;
const MAX_LOG_LIMIT = 100;

// Fetches a consultant's most recent QA credits (newest first) — each with the
// report title, client, findings count and the moment the QA was performed. Thin
// wrapper over the store so the /qa-logs command has a single entry point (and the
// limit default lives in one place).
async function buildUserLogs(actorCuid, limit = DEFAULT_LOG_LIMIT) {
  return kpiStore.recentByActor(actorCuid, limit);
}

// Formats a Date as "4 Aug, 14:32" in UK local time (Cognisys is UK-based, so the
// wall-clock time a consultant recognises), or "unknown time" when missing.
...
// "12 findings" / "1 finding", or "findings unknown" when the count wasn't recorded
// (kept distinct from a genuine "0 findings").
function findingsLabel(n) {
  if (!Number.isFinite(n)) return 'findings unknown';
  return `${n} ${n === 1 ? 'finding' : 'findings'}`;
}

// Renders a consultant's recent QA log as Slack mrkdwn. Pure — no I/O. Each entry is
// a single line: "timestamp - report title - client - findings". Newest first (as
// supplied); an unknown client is omitted so the line stays clean.
function renderUserLogs(displayName, logs) {
  const base = `*QA logs — ${slackEscape(displayName)}*`;
  if (!logs || logs.length === 0) {
    return `${base}\n\n_No QA activity recorded for this consultant._`;
  }
  const lines = logs.map((e, i) => {
    const title = slackEscape(e.reportName || `Report ${e.reportId}`);
    const parts = [formatDateTime(e.countedAt), title];
    if (e.clientName) parts.push(slackEscape(e.clientName));
    parts.push(findingsLabel(e.findingsCount));
    return `${i + 1}. ${parts.join(' - ')}`;
  });
  return [
    `${base} — ${logs.length} most recent`,
    '',
    ...lines,
  ].join('\n');
}
...
module.exports = {
  buildKpiEntries, renderKpis, parseWindow, slackEscape, formatDateTime,
  parseUserMention, buildUserPeriods, renderUserStats,
  buildUserLogs, renderUserLogs, DEFAULT_LOG_LIMIT, MAX_LOG_LIMIT,
};
