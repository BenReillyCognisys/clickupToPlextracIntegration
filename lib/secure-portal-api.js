 ...
 * The response also carries the task's test-files upload link — one intake call
 * fills both ClickUp fields. When the portal couldn't mint a files link the two
 * testFiles* keys are simply absent; that means "leave the field alone and try again
 * on the next sync", not an intake failure.
 *
 * Returns the parsed body:
 *   { ok, formUrl, formToken, created, testFilesUrl?, testFilesToken? }.
 */
...
/**
 * POST /api/clickup/test-files — create-or-return the per-task upload link clients
 * use to send us the files an engagement needs (source archives, VPN packs, sample
 * data). Idempotent on clickupTaskId — one link for the life of the task — so it is
 * safe to call on every sync, and `created` says whether the link is new.
 *
 * Only needed for a task that wants a files link but never gets an auth form: for
 * everything else createAuthForm() already returns the same link.
 *
 * Returns the parsed body:
 *   { ok, testFilesUrl, testFilesToken, created, authFormUrl? }.
 */
async function createTestFilesLink(payload) {
  return portalPost('/api/clickup/test-files', payload);
}
...
module.exports = { createAuthForm, updateAuthForm, createTestFilesLink };
