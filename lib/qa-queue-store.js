...
// order each section oldest-first (longest-waiting at the top); `stage` /
...
// Returns every queued report, oldest-first (by entered_at) — the report that has
// been waiting in the queue longest at the top.
async function list() {
  const c = await col();
