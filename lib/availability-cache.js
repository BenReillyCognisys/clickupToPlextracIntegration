...
// Internal-audit roster comes from a ClickUp view's pinned sidebar assignees —
// exactly the pentest mechanism (CLICKUP_VIEW_ID), just a different view. Unlike
// pentest there is no service-type/skill filtering: availability is searched
// purely on day count across the view's assignees.
const INTERNAL_AUDIT_VIEW_ID = process.env.CLICKUP_INTERNAL_AUDIT_VIEW_ID || '6-901507566408-18';
...
// Reads a roster ({ userId: name }) from a view's pinned sidebar assignees.
// viewId defaults to the pentest view; the internal-audit fetcher passes its own.
// The fallback defaults to {} so CLICKUP_ROSTER_FALLBACK (passed only for pentest)
// never leaks into another view's roster.
async function getRoster(membersMap, viewId = CLICKUP_VIEW_ID, fallback = {}) {
  const data = await clickupGet(CLICKUP_V2, `/view/${viewId}`);
  ...
  if (!ids.length) {
    if (Object.keys(fallback).length) {
      return Object.fromEntries(
        Object.entries(fallback).map(([id, n]) => [String(id), n])
      );
    }
    throw new Error(
      `Couldn't find any pinned assignees on view ${viewId}. Pin people to the view's sidebar`
      + (viewId === CLICKUP_VIEW_ID ? ', or set CLICKUP_ROSTER_FALLBACK in .env.' : '.')
    );
  }
...
// Names of the ClickUp custom field(s) holding a task's effort in days (e.g. 0.5
// for a half-day job). Drives the fractional per-day load used for half-day
// availability (the Free Black Box Test). Candidates are tried in order, so the
// primary field wins; override/extend via CLICKUP_LOAD_FIELD_NAMES (comma list).
const LOAD_FIELD_NAMES = (process.env.CLICKUP_LOAD_FIELD_NAMES || 'Days, Days Balance')
  .split(',')
  .map((s) => s.trim().toLowerCase())
  .filter(Boolean);

// Reads the numeric effort (days) custom field off a task, or null if none of the
// candidate fields carry a parseable number.
function taskDaysBalance(task) {
  const fields = task.custom_fields || [];
  for (const want of LOAD_FIELD_NAMES) {
    for (const f of fields) {
      if ((f.name || '').trim().toLowerCase() === want) {
...
async function computeAvailability(roster, membersMap) {
  ...
  // busy[ymd]  → Set of names with ANY commitment that day (whole-day model).
  // load[ymd]  → { name: fractional days committed } (half-day model). A day where
  //              a consultant's load is <= 0.5 still has >= 0.5 day free.
  const busy = {};
  const load = {};
  for (const ymd of windowDays) { busy[ymd] = new Set(); load[ymd] = {}; }

  // Diagnostics for half-day detection — surfaced via /availability/freeblackbox?debug=1.
  const loadStats = {
    tasks_total:             tasks.length,
    tasks_assigned_in_view:  0, // assigned to a roster member AND dated
    tasks_with_days_balance: 0, // of those, how many carried a numeric effort value
    load_field_candidates:   LOAD_FIELD_NAMES,
    sample_custom_fields:    [], // field names seen on the first few tasks (debug aid)
  };

  for (const task of tasks) {
    if (loadStats.sample_custom_fields.length < 3 && (task.custom_fields || []).length) {
      loadStats.sample_custom_fields.push((task.custom_fields || []).map((f) => f.name));
    }
    ...
    loadStats.tasks_assigned_in_view++;
    ...
    // Fractional per-day load: spread the task's Days Balance evenly across the
    // weekdays it spans. A missing/zero balance falls back to a full day (1.0) so
    // the day reads as fully busy — matching the boolean busy logic above.
    const rangeWeekdays = days.filter((ymd) => {
      const dow = ymdToUTC(ymd).getUTCDay();
      return dow >= 1 && dow <= 5;
    });
    const db     = taskDaysBalance(task);
    if (db != null) loadStats.tasks_with_days_balance++;
    const perDay = db != null && db > 0 ? db / (rangeWeekdays.length || 1) : 1;
    for (const ymd of rangeWeekdays) {
      if (load[ymd]) for (const n of names) load[ymd][n] = (load[ymd][n] || 0) + perDay;
    }
  }
...
// Pentest roster comes from the team view sidebar (CLICKUP_ROSTER_FALLBACK applies).
async function fetchAvailability(membersMap) {
  if (!membersMap) membersMap = await getMembersMap();
  const roster = await getRoster(membersMap, CLICKUP_VIEW_ID, ROSTER_FALLBACK);
  return computeAvailability(roster, membersMap);
}

// Internal-audit roster comes from its own view's pinned sidebar assignees.
async function fetchInternalAuditAvailability(membersMap) {
  if (!membersMap) membersMap = await getMembersMap();
  const roster = await getRoster(membersMap, INTERNAL_AUDIT_VIEW_ID);
  return computeAvailability(roster, membersMap);
}
...
// Lowercase and collapse any run of non-alphanumeric characters (slashes,
// hyphens, extra spaces) to a single space so cosmetic separator differences
// between the frontend label and the Doc entry don't break matching, e.g.
// "Code Review / White-Box" and "Code Review/White Box" both normalize to
...
function matchServiceType(query, serviceTypesPayload) {
  const q   = normalizeServiceType(query);
  const all = serviceTypesPayload.service_types || [];
  const exact = all.filter((s) => normalizeServiceType(s.service_type) === q);
  if (exact.length === 1) return { match: exact[0], all };
  // Bidirectional containment: matches whether the frontend sends a broader or a
  // more specific label than the canonical Doc name.
  const subs  = all.filter((s) => {
    const n = normalizeServiceType(s.service_type);
...
function findFieldByName(fields, wantedName) {
  ...
  const exact = fields.filter((f) => (f.name || '').trim().toLowerCase() === want);
  if (exact.length === 1) return exact[0];
  const subs  = fields.filter((f) => (f.name || '').trim().toLowerCase().includes(want));
...
// Constant-time comparison of the supplied key against the configured one. Mirrors
// the timing-safe check used for the webhook HMAC signatures: lengths are compared
// first (timingSafeEqual throws on a length mismatch), then the bytes — so the
