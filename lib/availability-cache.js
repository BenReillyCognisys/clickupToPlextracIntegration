...
// ClickUp List view whose sidebar assignees form the internal-audit roster — the
// same mechanism the pentest roster uses (CLICKUP_VIEW_ID), just a different view.
// Unlike pentest, internal-audit availability has no service-type/skill filtering;
// it searches purely on day count across this view's assignees.
...
// Reads a roster ({ userId: name }) from a ClickUp view's sidebar assignees.
// viewId defaults to the pentest view; the internal-audit fetcher passes its own
// view id. The CLICKUP_ROSTER_FALLBACK only applies to the pentest view (fallback
// defaults to {} so it never leaks into a different view's roster).
async function getRoster(membersMap, viewId = CLICKUP_VIEW_ID, fallback = {}) {
  ...
  if (!ids.length) {
    ...
    throw new Error(
      `Couldn't find any assignees on view ${viewId}. Add people to the view's sidebar`
...
// Internal-audit roster comes from its own List view's sidebar assignees.
