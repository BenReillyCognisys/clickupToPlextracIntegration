...
// Internal-audit roster = the MEMBERS of the list behind this ClickUp List view.
// The list id is derived from the view's parent; set CLICKUP_INTERNAL_AUDIT_LIST_ID
// to skip the view lookup and name the list directly. Unlike pentest there is no
// service-type/skill filtering — availability is searched purely on day count
// across the list's members.
const INTERNAL_AUDIT_VIEW_ID = process.env.CLICKUP_INTERNAL_AUDIT_VIEW_ID || '8cnj048-61655';
const INTERNAL_AUDIT_LIST_ID = process.env.CLICKUP_INTERNAL_AUDIT_LIST_ID || '';
...
// Resolves the internal-audit list id: either the explicit env override, or the
// parent list of the configured List view (8cnj048-61655 is a view, not a list).
async function getInternalAuditListId() {
  if (INTERNAL_AUDIT_LIST_ID) return INTERNAL_AUDIT_LIST_ID;
  const data   = await clickupGet(CLICKUP_V2, `/view/${INTERNAL_AUDIT_VIEW_ID}`);
  const parent = data.view?.parent || {};
  if (!parent.id) {
    throw new Error(
      `Could not resolve a list id from view ${INTERNAL_AUDIT_VIEW_ID} (no parent). `
      + 'Set CLICKUP_INTERNAL_AUDIT_LIST_ID to the list id directly.'
    );
  }
  return String(parent.id);
}

// Internal-audit roster = the members of the internal-audit list.
async function getInternalAuditRoster(membersMap) {
  const listId  = await getInternalAuditListId();
  const data    = await clickupGet(CLICKUP_V2, `/list/${listId}/member`);
  const members = data.members || [];
  if (!members.length) {
    throw new Error(`Internal-audit list ${listId} has no members to use as the roster`);
  }
  const roster = {};
  for (const m of members) {
    const id = String(m.id);
    roster[id] = m.username || m.email || membersMap[id]?.name || `User ${id}`;
  }
  return roster;
}
