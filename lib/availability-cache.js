...
// Fetches a single task, accepting either a native ClickUp task id or a custom
// task id. ClickUp returns OAUTH_027 ("Team not authorized") both for genuine
// auth failures AND for identifiers it can't resolve, so we try a native lookup
// first and a custom-id lookup second, surfacing BOTH failures if neither works
// (rather than swallowing the first and reporting only the second).
async function fetchClickupTask(taskId) {
  const attempts = [
    { label: 'native id',       params: null },
    { label: 'custom task id',  params: [['custom_task_ids', 'true'], ['team_id', CLICKUP_TEAM_ID]] },
  ];
  const errors = [];
  for (const { label, params } of attempts) {
    try {
      return await clickupGet(CLICKUP_V2, `/task/${taskId}`, params);
    } catch (err) {
      errors.push(`[${label}] ${err.message}`);
    }
  }
  throw new Error(`Could not fetch task "${taskId}" as a native or custom id — ${errors.join('  ||  ')}`);
}
...
async function getInternalAuditRoster(membersMap) {
  ...
  const task = await fetchClickupTask(INTERNAL_AUDIT_TASK_ID);
