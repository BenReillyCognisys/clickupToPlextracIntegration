...
// Name of the ClickUp custom field holding a task's effort in days (e.g. 0.5 for
// a half-day job). Drives the fractional per-day load used for half-day
// availability (the Free Black Box Test). Same field the scheduler writes to.
const DAYS_BALANCE_FIELD = (process.env.CLICKUP_DAYS_FIELD_NAME || 'Days Balance').trim().toLowerCase();

// Reads the numeric "Days Balance" custom field off a task, or null if absent.
function taskDaysBalance(task) {
  for (const f of (task.custom_fields || [])) {
    if ((f.name || '').trim().toLowerCase() === DAYS_BALANCE_FIELD) {
...
async function computeAvailability(roster, membersMap) {
  ...
  const loadStats = {
    ...
    tasks_with_days_balance: 0, // of those, how many carried a numeric Days Balance
    days_balance_field:      process.env.CLICKUP_DAYS_FIELD_NAME || 'Days Balance',
