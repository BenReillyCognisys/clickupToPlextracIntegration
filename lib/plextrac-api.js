...
//
// ⚠️ UNVERIFIED against the live instance. The path below is Plextrac's documented
// v1 export route; instances differ, and some versions return a JSON job/link body
// instead of the file itself. Confirm with `node scripts/inspect-export.js` and, if
// it differs, override PLEXTRAC_EXPORT_PATH (the literal `{clientId}`, `{reportId}`
// and `{format}` tokens are substituted) rather than editing this.
