...
//
// The exact path/response shape varies by Plextrac version; the default below is
// the tenant-scoped users endpoint. Override the path with PLEXTRAC_USERS_PATH
// (use the literal `{tenantId}` token where the id belongs) if your instance
// differs — confirm with `node scripts/inspect-users.js`.
async function listTenantUsers() {
  const tid = await tenantId();
  const path = (process.env.PLEXTRAC_USERS_PATH || '/api/v1/tenant/{tenantId}/users')
